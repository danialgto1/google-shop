...
def google_shop(input_query_model):
    ...
    while (page_counter<product_qty//20):
        ...
        with open ('l.html' , 'w') as file:
            file.writelines(page.text)        
        elements = soup.find_all('div' , class_='i0X6df')
        if not len(elements):
            return ('Not Found')
            break
...
products = google_shop(input_query_model)
try:
    print([product['website'] for product in products ])
except:
    print(products)
